...
void CameraCalibratorNode::load_config(const std::string& config_path) {
  ...
  // Override tof1 intrinsic path to fixed path
  config_.camera_intrinsics["tof1"] = "config/cali_result_intrinsic.yaml";
...
void CameraCalibratorNode::load_camera_info() {
  for (const auto& [cam_id, info_path] : config_.camera_intrinsics) {
    ...
    CameraInfo cam_info;
    cam_info.camera_matrix = cv::Mat::eye(3, 3, CV_64F);
    cam_info.dist_coeffs = cv::Mat::zeros(5, 1, CV_64F);

    if (cam_id == "tof1") {
      // Load from FIRST_CAMERA_PARAMS in config/cali_result_intrinsic.yaml
      auto params = info["FIRST_CAMERA_PARAMS"];
      double fx = params["Fx"].as<double>();
      double fy = params["Fy"].as<double>();
      double cx = params["Cx"].as<double>();
      double cy = params["Cy"].as<double>();

      // Correction for 240x180 image size (original height 100)
      cy += 40.0;

      cam_info.camera_matrix.at<double>(0, 0) = fx;
      cam_info.camera_matrix.at<double>(1, 1) = fy;
      cam_info.camera_matrix.at<double>(0, 2) = cx;
      cam_info.camera_matrix.at<double>(1, 2) = cy;

      // Distortion: k1, k2, p1, p2, k3
      cam_info.dist_coeffs.at<double>(0) = params["K1"].as<double>();
      cam_info.dist_coeffs.at<double>(1) = params["K2"].as<double>();
      cam_info.dist_coeffs.at<double>(2) = params["P1"].as<double>();
      cam_info.dist_coeffs.at<double>(3) = params["P2"].as<double>();
      cam_info.dist_coeffs.at<double>(4) = params["K3"].as<double>();
    } else if (cam_id == "tof2") {
      // Load from FIRST_CAMERA_PARAMS in the file specified for tof2
      // Note: User instruction says tof2's intrinsic file also has
      // FIRST_CAMERA_PARAMS structure
      auto params = info["FIRST_CAMERA_PARAMS"];
      double fx = params["Fx"].as<double>();
      double fy = params["Fy"].as<double>();
      double cx = params["Cx"].as<double>();
      ...
      // Fallback to original loading logic for other cameras
      auto K = info["camera_matrix"]["data"].as<std::vector<double>>();
      for (int i = 0; i < 9; ++i)
        cam_info.camera_matrix.at<double>(i / 3, i % 3) = K[i];

      auto D =
          info["distortion_coefficients"]["data"].as<std::vector<double>>();
...
void CameraCalibratorNode::save_visualization(const cv::Mat& image,
  ...
  if (found) {
    ...
    // Since we are drawing on an undistorted image, distortion coefficients
    // are zero
    ...
    try {
      ...
      // Fallback for older OpenCV versions if drawFrameAxes is not available
      // or fails But drawFrameAxes was introduced in 3.4.1, which is quite
      // old.
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<calib_3dtof::CameraCalibratorNode>();
  node->run();
