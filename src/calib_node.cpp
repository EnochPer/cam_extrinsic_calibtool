...
namespace calib_3dtof {
  ...
  CameraCalibratorNode::CameraCalibratorNode(const rclcpp::NodeOptions& options)
    ...
    this->declare_parameter<std::string>("config_path", "config/extrinsic_config.yaml");
  }
  ...
  void CameraCalibratorNode::process_data() {
    ...
    for (const auto& data : config_.data) {
      ...
      if (cameras_.find(cam_id) == cameras_.end()) {
        RCLCPP_ERROR(this->get_logger(), "Unknown camera ID: %s",
                     cam_id.c_str());
        continue;
      }
      ...
      if (detect_corners(image, corners)) {
        ...
        if (solve_pnp(corners, cam_info, T_cam_board, rvec, tvec)) {
          ...
          RCLCPP_INFO(this->get_logger(),
                      "Frame %s: Valid pose found and saved.",
                      filename.c_str());
        } else {
  ...
  bool CameraCalibratorNode::solve_pnp(
      const std::vector<cv::Point2f>& corners, const CameraInfo& cam_info,
      Eigen::Affine3d& T_cam_board, cv::Mat& rvec, cv::Mat& tvec) {
  ...
  void CameraCalibratorNode::save_visualization(
      const cv::Mat& image, const std::string& filename,
      const CameraInfo& cam_info, const cv::Mat& rvec, const cv::Mat& tvec) {
    namespace fs = std::filesystem;
